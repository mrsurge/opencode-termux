...
- [x] Accept a second `turn/start` while a session has active provider work by holding it in the stdio app-server queue and admitting it through `prompt_async` only after the current route-backed turn completes. This preserves one ALS turn per user prompt instead of letting OpenCode's route runner coalesce the interjection into the active prompt loop.
...
- [x] Preserve OpenCode's native context-overflow compaction handoff. Route-backed `ContextOverflowError` / provider `context_length_exceeded` errors now emit a nonterminal `turn/warning`, keep the active turn open for OpenCode's own compaction loop, expose manual `session/compact` through the existing `/session/:sessionID/summarize` route, suppress summary assistant messages as normal assistant output, and map compaction completion to ALS `context_compacted` live/replay rows.
- [x] Strengthen OpenCode compaction handoff prompts so repeated compactions preserve conversation narrative and still-relevant prior-compaction carryover instead of flattening the session into only a task ledger.
