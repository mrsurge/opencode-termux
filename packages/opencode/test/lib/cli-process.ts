...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
      ...
      yield* Effect.forkScoped(
        fromBunStream("stdout", () => proc.stdout).pipe(
          ...
          Stream.runForEach((line) => {
            if (line.length === 0) return Effect.void
            try {
              return Queue.offer(responses, JSON.parse(line) as unknown)
            } catch {
              return Queue.offer(responses, { _rawLine: line })
            }
          }),
          Effect.ignore({ log: true }),
        ),
    ...
    const opencode: OpencodeCli = { run, serve, acp, appServer, spawn, expectExit, parseJsonEvents }
